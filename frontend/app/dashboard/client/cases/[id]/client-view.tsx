      ...
      <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <h1 className="text-3xl font-extrabold text-slate-900">{caseData.title}</h1>
          </div>
              ...
              <div className="pt-12 px-6 pb-6">
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex items-center gap-3 text-slate-600">
                    ...
                    <span>{lawyer.lawyerProfile?.mobileNumber || "Not available"}</span>
                  </div>
                  <div className="flex items-start gap-3 text-slate-600">
                    <MapPin size={16} className="text-slate-400 mt-0.5" />
                    <span>{lawyer.lawyerProfile?.chamberAddress || "Chamber address not listed"}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
            <div className="flex justify-between items-center mb-8">
              ...
              {/* Optional: Add Event button for client? Usually clients don't add timeline events, only documents */}
            </div>
              ...
              {events.map((event: any) => {
                ...
                return (
                  <div key={event.id} className="relative pl-10 group">
                    ...
                    {/* Event Card */}
                    <div className={`rounded-xl border p-6 transition ${isUpcoming ? 'bg-blue-50/40 border-blue-200' : 'bg-white border-slate-200'}`}>
                        ...
                        {event.attachments && event.attachments.length > 0 && (
                          <div className="flex flex-wrap gap-2 pt-2">
                            {event.attachments.map((url: string, idx: number) => (
                              <a key={idx} href={url} target="_blank" className="flex items-center gap-2 bg-white border border-slate-200 px-3 py-1.5 rounded-lg text-xs font-bold text-slate-700 hover:border-blue-400 hover:text-blue-700 transition">
                                {getFileIcon(url)}
